...
```bash
java -Duser.timezone=UTC -jar ojp-server/target/ojp-server-0.4.3-beta-shaded.jar
```
...
```bash
# Start in background
nohup java -Duser.timezone=UTC -jar ojp-server-0.4.3-beta-shaded.jar \
  > ojp-server.log 2>&1 &
...
# Start OJP Server
docker run --network ojp-network --name ojp-server rrobetti/ojp:0.4.3-beta
