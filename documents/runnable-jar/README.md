...
```bash
wget https://repo1.maven.org/maven2/org/openjproxy/ojp-server/0.4.2-beta/ojp-server-0.4.2-beta-shaded.jar
```

Or using `curl`:

```bash
curl -LO https://repo1.maven.org/maven2/org/openjproxy/ojp-server/0.4.2-beta/ojp-server-0.4.2-beta-shaded.jar
```

### 2. Make the JAR Executable (Optional)

```bash
chmod +x ojp-server-0.4.2-beta-shaded.jar
```
...
java -Duser.timezone=UTC -jar ojp-server-0.4.2-beta-shaded.jar

# Or specify custom path
java -Duser.timezone=UTC -Dojp.libs.path=./ojp-libs -jar ojp-server-0.4.2-beta-shaded.jar
```
...
```bash
java -Duser.timezone=UTC -Dojp.libs.path=/opt/drivers -jar ojp-server-0.4.2-beta-shaded.jar
```
...
java -Duser.timezone=UTC \
     ...
     -jar ojp-server-0.4.2-beta-shaded.jar
```
...
```bash
nohup java -Duser.timezone=UTC -jar ojp-server-0.4.2-beta-shaded.jar > ojp-server.log 2>&1 &
```
...
```bash
java -Duser.timezone=UTC -Xmx2g -jar ojp-server-0.4.2-beta-shaded.jar
```
